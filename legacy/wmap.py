...
class Wmap(Map):
    def __init__(self, world, wx, wy, width, height):
        ...
        self.width = width
        self.height = height
        self.center = int(round(self.width / 2)), int(round(self.height / 2))
    ...
    def set_initial_dmaps(self):
        ...
        self.add_dmap(key='map_center', target_nodes=[ self.center ], dmrange=5000)
    ...
    def make_city_map(self, site, num_nodes, min_dist, disorg):
        #Make a city on this tile
        self.city_map_generator = CityMapGenerator(self, site, self.world.tiles[site.x][site.y].entities)
        self.city_map_generator.generate_city_map(num_nodes=num_nodes, min_dist=min_dist, disorg=disorg)
    ...
    def add_sapients_to_map(self, entities, populations, place_anywhere=0):
        ...
        # for good_producer in self.site.econ.good_producers:
        ...
        #         new_building = self.site.create_building(zone='commercial', type_=profession_to_business[ptype],
        ...
        #
        # taverns = [building for building in self.site.buildings if building.type_ == 'Tavern']
        # #for figure in self.site.figures:
    ...
    def add_world_features(self, x, y):
        ...
        if self.world.tiles[x][y].has_feature('road'):
            ...
            while len(road_start_positions) >= 2:
                ...
                r_begin.connect(other=r_end, n_div=5, div_mag=1, wmap=self)
            ...
            for y in range(self.height):
                for x in range(self.width):
                    if self.tiles[x][y].zone in (None, 'wilderness') and self.tiles[x][y].surface == 'ground':
                        ...
                        for (xx, yy) in get_border_tiles_8(x, y):
...
class PathNode:
    ...
    def connect(self, other, n_div, div_mag, wmap):
        ...
        for n in range(len(coords) - 1):
            ...
            ## Color self ##
            #wmap.tiles[self.x][self.y].color = libtcod.cyan
            wmap.tiles[self.x][self.y].make_road('paved')
...
class CityMapGenerator:
    def __init__(self, wmap, site, figures):
        self.site = site
        self.wmap = wmap
    ...
    def add_initial_nodes(self, num_nodes, min_dist):
        ...
        while new_nodes < num_nodes:
            ...
            use_node = False
            while not use_node:
                nx, ny = roll(10, self.wmap.width - 11), roll(10, self.wmap.height - 11)
        ...
        for node in self.all_nodes:
            ...
            for neighbor in node.neighbors:
                if not neighbor in node.connected:
                    node.connect(other=neighbor, n_div=3, div_mag=1, wmap=self.wmap)

    def generate_city_map(self, num_nodes, min_dist, disorg):
        steps = 9
        pbarname = 'Generating ' + self.site.name + ' map'
        g.game.render_handler.progressbar_screen(pbarname, 'creating main avenues', 1, steps)
        #### Generate the city!
        self.make_main_avenues(block_size=min_dist, disorg=disorg)
        ...
        begin = time.time()
        outside_tiles = floodfill(fmap=self.wmap, x=1, y=1, do_fill=do_fill, do_fill_args=[], is_border=lambda tile: tile.zone in border_cells)
        ...
        for development_option in development_options:
            ...
            elif get_distance_to(distx, disty, int(round(self.wmap.width / 2)), int(round(self.wmap.height / 2))) > ( (self.wmap.height + self.wmap.width) / 2) / 4:
    ...
    def make_market(self, empty_lot, zone):
        ...
        for x, y in lot:
            if self.wmap.tiles[x][y].surface not in DEVELOPED_SURFACES:
                self.wmap.tiles[x][y].surface = 'floor' # Quick hack - for some reason the land will develop if we don't turn it into floor
                self.wmap.tiles[x][y].colorize(libtcod.dark_sepia)
        ...
        while undeveloped_spaces != []:
            ...
            if usable:
                ...
                while expansion_dirs != []:
                    ...
                    if not 2 < newrect.y1 - 1 < self.wmap.height - 2:
                        expansion_dirs.remove('n')
                    if not 2 < newrect.y2 + 1 < self.wmap.height - 2:
                        expansion_dirs.remove('s')
                    if not 2 < newrect.x1 - 1 < self.wmap.width - 2:
                        expansion_dirs.remove('w')
                    if not 2 < newrect.x2 + 1 < self.wmap.width - 2:
                ...
                for q in range(newrect.x1, newrect.x2 + 1):
                    for r in range(newrect.y1, newrect.y2 + 1):
                        ...
                        if q in [newrect.x1, newrect.x2] or r in [newrect.y1, newrect.y2]:
                            ...
                            self.wmap.tiles[q][r].make_floor(floor_type=floor)
                ...
                for mpx, mpy in (newrect.middle_point('n'), newrect.middle_point('s'), newrect.middle_point('e'), newrect.middle_point('w')):
                    if 'road' in (self.wmap.tiles[mpx - 1][mpy].zone, self.wmap.tiles[mpx + 1][mpy].zone, self.wmap.tiles[mpx][mpy - 1].zone, self.wmap.tiles[mpx][mpy + 1].zone):
                        ### DOOR!
                        self.wmap.make_door(x=mpx, y=mpy, floor_type=floor)
                        has_door = True
                        break
                '''
                if not has_door:
                    ...
                    for (dx, dy), (mpx, mpy) in directions.items():
                        ...
                        while iter != 10:
                            ...
                            if self.wmap.tiles[px + dx][py + dy].surface == 'road':
                                self.wmap.make_door(x=mpx, y=mpy, floor_type=floor)
                                for tx, ty in path:
                                    self.wmap.tiles[tx][ty].make_road('paved')
                                    #### DOOR!
                                has_door = True
                                break

                            ## Second possibility: we run into something we don't want, and must stop to try another direction
                            elif self.wmap.tiles[px + dx][py + dy].surface in DEVELOPED_SURFACES:
                                break
                ...
                if has_door:
                    if zone == 'residential' and newrect.get_size() < 80:
                        ...
                        #self.wmap.tiles[x][y].color = libtcod.black

        for x, y in empty_lot:
            if self.wmap.tiles[x][y].surface not in DEVELOPED_SURFACES:
                self.wmap.tiles[x][y].colorize(libtcod.darker_sepia)
                ## Only place these items near a wall
                near_wall = 0
                for (tx, ty) in get_border_tiles(x, y):
                    if self.wmap.is_val_xy(coords=(tx, ty)) and self.wmap.tiles[tx][ty].surface == 'wall':
                        near_wall = 1
                        break
                ...
                if near_wall:
                    num = roll(1, 30)
                    if num == 1:
                        self.wmap.create_and_add_object(name='crate', x=x, y=y)
                    elif num == 2:
                        self.wmap.create_and_add_object(name='barrel', x=x, y=y)
                    elif num == 3:
                        self.wmap.create_and_add_object(name='pottery', x=x, y=y)
    ...
    def historize_buildings(self):
        ## Add in historical figures from the city info to the city
        market = self.site.get_building('Market')
        if market:
            for x, y in self.markets[0]:
                self.wmap.tiles[x][y].building = market
                ...
                if potential_buildings:
                    ...
                    if building.linked_economy_agent:
                        ...
                        if amount:
                            ...
                            obj = it.assemble_object(object_blueprint=phys.object_dict['crate'], force_material=material, wx=self.wmap.wx, wy=self.wmap.wy)
                            ...
                            for n_obj in building.linked_economy_agent.get_sold_objects():
                                n_obj = it.assemble_object(object_blueprint=phys.object_dict[n_obj], force_material=material, wx=self.wmap.wx, wy=self.wmap.wy)
        ...
        for i in range(x, x + w):
            for j in range(y, y + h):
                ...
                if i in [x, x + w - 1] or j in [y, y + h - 1]:
                    self.wmap.tiles[i][j].make_wall(libtcod.darkest_grey)
                else:
                    self.wmap.tiles[i][j].make_floor(floor_type='stone')

        door_x, door_y = int(round((x + (x + w)) / 2)), y + h - 1

        self.wmap.make_door(x=door_x, y=door_y, floor_type='stone')
    ...
    def make_main_avenues(self, block_size, disorg):
        ...
        building = self.site.get_building('City Hall')
        if building:
            self.make_municipal_bldg(x=self.wmap.center[0], y=self.wmap.center[1], w=block_size, h=block_size, road_thickness=road_thickness, building_info=building)

        temple = self.site.get_building('Temple')
        if temple:
            self.make_municipal_bldg(x=self.wmap.center[0]-60, y=self.wmap.center[1], w=block_size, h=block_size, road_thickness=road_thickness, building_info=temple)
        ...
        for node in self.main_aves:
            for neighbor in node.neighbors:
                node.connect(other=neighbor, n_div=3, div_mag=2, wmap=self.wmap)
        '''

        road_thick = 2
        sx, sy = self.wmap.center[0] - xlen, self.wmap.center[1] - ylen
